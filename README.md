...
```
bower install vimbotour
```
Alternative way:
- Download the latest version of VimboTour
- Extract the archive with VimboTour.
- Move the VimboTour directory to somewhere on your webserver
- Insert next lines into your page's \<head\> tag:
```html
  <link href="<pathontheserver>/vimbotour/vimbotour.css" rel="stylesheet">
  <script src="<pathontheserver>/vimbotour/vimbotour.min.js"></script>
```

#### Initialization and configuration:
```javascript
//initialize instance
var vimbotour_instance = new VimboTour({});

//simple config. 
//Only one step - highlighting(with description) "New" button 
//hide VimboTour after a click on the button.
var vimbotour_script_steps = [
  {
...
//set script config
vimbotour_instance.set(vimbotour_script_steps);

//run vimbotour script
vimbotour_instance.run();
```
...
* `keyCode` - the code of a button, which triggers the next VimboTour step upon a click. Defined by the “key” event. (“key #block” : “hello”).
...
* `custom` - this value is very usefull if you need to go to the next step by event in your app code. For example, you want to go to the next step only after some data have been loaded in your application. Then you should use the "custom" event_type and the "trigger" method of the VimboTour instance.  
```javascript
...
$.get('/load/some_data', function(data){
  ...
  vimbotour_instance.trigger('custom_event_name');
});
```  
* `next` - when you set value of event_type to "next", you will see the "Next" btn on this step.
* `key` - tells VimboTour to go to the next step when you click on the button defined by the keyCode
...
* New and simplified description of VimboTour steps
...
* New examples
* You can learn the step you are on by the class vimbotour-step-* ( where * stands for the step number).
